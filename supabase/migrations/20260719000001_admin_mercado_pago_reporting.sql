...
-- ----------------------------------------------------------------
-- Keep ticket_types.sold_quantity synchronized with real approved tickets.
-- This removes demo/seed counters from the existing dashboard chart.
-- ----------------------------------------------------------------
...
begin
  update public.ticket_types tt
     set sold_quantity = src.sold_count,
         updated_at = now()
    from (
      select
        tt2.id as ticket_type_id,
        count(t.id) filter (where o.id is not null)::integer as sold_count
   ...
   where tt.id = src.ticket_type_id
     and tt.sold_quantity is distinct from src.sold_count;
...
begin
  ...
  perform public.refresh_ticket_type_sold_quantity(v_event_id);
...
after insert or delete or update of order_id, ticket_type_id
on public.tickets
...
-- ----------------------------------------------------------------
-- Mercado Pago-aware report consumed by dashboard and reports page.
-- Values remain numeric because the current UI formats generic JSON metrics.
-- ----------------------------------------------------------------
...
begin
  ...
  select jsonb_build_object(
    ...
    'participants_approved', coalesce((
      select count(*) from public.order_participants op
      join public.orders o on o.id = op.order_id
      where o.event_id = p_event_id
        and o.payment_status::text = 'approved'
        and op.status in ('active', 'reserved')
    ), 0),
    'revenue_cents', coalesce((select sum(total_amount_cents) from public.orders where event_id = p_event_id and payment_status::text = 'approved'), 0),
    'subtotal_cents', coalesce((select sum(subtotal_amount_cents) from public.orders where event_id = p_event_id and payment_status::text = 'approved'), 0),
    'extras_revenue_cents', coalesce((select sum(extras_amount_cents) from public.orders where event_id = p_event_id and payment_status::text = 'approved'), 0),
    'average_order_cents', coalesce((select round(avg(total_amount_cents))::bigint from public.orders where event_id = p_event_id and payment_status::text = 'approved'), 0),
    'orders_total', coalesce((select count(*) from public.orders where event_id = p_event_id), 0),
    'orders_pending', coalesce((select count(*) from public.orders where event_id = p_event_id and payment_status::text in ('pending', 'in_process', 'authorized')), 0),
    ...
    'preferences_active', coalesce((select count(*) from public.payment_preferences pp join public.orders o on o.id = pp.order_id where o.event_id = p_event_id and pp.status = 'active'), 0),
    'preferences_expired', coalesce((select count(*) from public.payment_preferences pp join public.orders o on o.id = pp.order_id where o.event_id = p_event_id and pp.status = 'expired'), 0),
    'payment_events_total', coalesce((select count(*) from public.payment_events pe join public.orders o on o.id = pe.order_id where o.event_id = p_event_id), 0),
    'payment_events_failed', coalesce((select count(*) from public.payment_events pe join public.orders o on o.id = pe.order_id where o.event_id = p_event_id and pe.processing_status in ('failed', 'error')), 0),
    'notification_jobs_pending', coalesce((select count(*) from public.notification_jobs nj join public.orders o on o.id = nj.order_id where o.event_id = p_event_id and nj.status in ('pending', 'processing')), 0),
    'notification_jobs_failed', coalesce((select count(*) from public.notification_jobs nj join public.orders o on o.id = nj.order_id where o.event_id = p_event_id and nj.status in ('failed', 'cancelled')), 0),
    'drinks_packages', coalesce((select sum(pe.quantity) from public.participant_extras pe join public.orders o on o.id = pe.order_id where o.event_id = p_event_id and o.payment_status::text = 'approved' and pe.extra_type = 'drinks'), 0),
    'barbecue_packages', coalesce((select sum(pe.quantity) from public.participant_extras pe join public.orders o on o.id = pe.order_id where o.event_id = p_event_id and o.payment_status::text = 'approved' and pe.extra_type = 'barbecue'), 0),
    'vouchers_delivered', coalesce((select count(*) from public.participant_extras pe join public.orders o on o.id = pe.order_id where o.event_id = p_event_id and pe.physical_vouchers_delivered_at is not null), 0),
    'refund_requests_open', coalesce((select count(*) from public.refund_requests rr join public.orders o on o.id = rr.order_id where o.event_id = p_event_id and rr.status in ('requested', 'under_review', 'approved', 'processing')), 0),
    'refund_amount_cents', coalesce((select sum(rr.refund_amount_cents) from public.refund_requests rr join public.orders o on o.id = rr.order_id where o.event_id = p_event_id and rr.status = 'refunded'), 0),
    'transfers_open', coalesce((select count(*) from public.ticket_transfers tr join public.tickets t on t.id = tr.ticket_id join public.orders o on o.id = t.order_id where o.event_id = p_event_id and tr.status in ('requested', 'accepted', 'processing')), 0),
    'checkins_done', coalesce((select count(*) from public.tickets t join public.orders o on o.id = t.order_id where o.event_id = p_event_id and t.checked_in = true), 0),
    'checkins_pending', coalesce((select count(*) from public.tickets t join public.orders o on o.id = t.order_id where o.event_id = p_event_id and o.payment_status::text = 'approved' and t.checked_in = false), 0),
