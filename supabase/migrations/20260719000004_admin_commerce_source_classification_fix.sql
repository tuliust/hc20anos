-- ================================================================
-- Fix admin commerce source classification
-- ================================================================
-- Legacy orders may contain Mercado Pago provider IDs from the previous flow.
-- Classify the hardened checkout only through tables exclusive to the new
-- transactional model: order_participants, payment_preferences and
-- payment_events.
...
begin
  ...
  from (
    select
      o.*,
      (
        exists (select 1 from public.order_participants op where op.order_id = o.id)
    ...
    select jsonb_agg(
      order_row || jsonb_build_object(
        'commerce_source', case
          when coalesce((order_row->>'participant_count')::integer, 0) > 0
            or coalesce((order_row->>'webhook_events')::integer, 0) > 0
            or exists (
              select 1
              from public.payment_preferences pp
              where pp.order_id = (order_row->>'id')::uuid
            )
          then 'mercado_pago'
          else 'legacy'
        end,
        'data_quality_alert', case
          ...
          when coalesce(order_row->>'payment_status', '') = 'approved'
            and (
              coalesce((order_row->>'participant_count')::integer, 0) > 0
              or coalesce((order_row->>'webhook_events')::integer, 0) > 0
              or exists (
                select 1
                from public.payment_preferences pp
            ...
            and coalesce((order_row->>'participant_count')::integer, 0) = 0
          then 'mercado_pago_approved_without_participants'
          else null
        end
      )
...
comment on function public.get_event_reports(uuid) is
  'Admin report with commerce source classification based on hardened checkout transactional evidence.';

comment on function public.get_admin_orders(text) is
  'Admin orders classified as legacy or Mercado Pago using participants, preferences and webhook events.';
