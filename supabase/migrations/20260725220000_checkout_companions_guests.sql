-- ================================================================
-- Checkout de acompanhantes e convidados no mesmo pedido.
--
-- Mantém preços e disponibilidade sob autoridade do lote vigente.
-- Permite convidados externos aprovados junto de ingressos de ex-aluno
-- e corrige a compra direta do convidado aprovado, que não possui um
-- participante do tipo alumni no próprio pedido.
-- ================================================================
...
declare
  ...
  v_included_external_slots_remaining integer := 0;
...
begin
  ...
  if p_product_code = 'external_guest' then
    if v_alumni_count <> 0 or v_spouse_count <> 0 or v_child_count <> 0 or v_external_count <> 1 then
  ...
  loop
    if v_participant->>'participant_type' = 'child' then
      ...
      where gar.event_id = v_event_id
        ...
        and lower(btrim(gar.guest_email)) = lower(btrim(v_participant->>'email'))
        and gar.status = 'approved'
      order by gar.decided_at desc nulls last, gar.created_at desc
      limit 1;
  ...
  v_included_external_slots_remaining := v_included_external;
  ...
  loop
    ...
    if v_participant->>'participant_type' = 'child' then
      ...
      if v_included_external_slots_remaining > 0 then
        v_included_external_slots_remaining := v_included_external_slots_remaining - 1;
      else
        v_participant_unit_price := v_external_guest.current_price_cents;
      end if;
    end if;
...
begin
  if position('v_included_external_slots_remaining' in v_definition) = 0 then
    raise exception 'external guest price allocation not installed';
  end if;
  if position('lower(btrim(gar.guest_email))' in v_definition) = 0 then
    raise exception 'approved guest e-mail validation not installed';
  end if;
  if position('p_product_code = ''external_guest''' in v_definition) = 0 then
    raise exception 'direct external guest composition not installed';
  end if;
end;
$$;
