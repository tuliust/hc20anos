...
create index if not exists security_audit_log_entity_idx
  on public.security_audit_log (entity_type, entity_id, created_at desc);
...
begin
  ...
  insert into public.security_audit_log(
    actor_user_id, actor_role, action, entity_type, entity_id, request_key, metadata_json
  ) values (
    auth.uid(), public.current_security_role(), btrim(p_action), btrim(p_entity_type),
    nullif(btrim(coalesce(p_entity_id, '')), ''), nullif(btrim(coalesce(p_request_key, '')), ''),
...
begin
  ...
  if v_count > p_limit then
    perform public.write_security_audit(
      'rate_limit_exceeded', 'rate_limit_bucket', v_bucket_key, p_action,
...
begin
  ...
  perform public.write_security_audit(lower(tg_op) || '_row', tg_table_name, v_entity_id, null, v_metadata);
  return coalesce(new, old);
end;
$$;

-- Registra mudanças de estado nas entidades operacionais sensíveis.
...
begin
  ...
  select jsonb_build_object(
    ...
    'tickets', (select jsonb_build_object(
      ...
      'checked_in', count(*) filter (where checked_in),
      'invalid', count(*) filter (where status in ('cancelled','refunded','transferred','chargeback'))
    ) from public.tickets),
    'transfers', (select jsonb_build_object(
      'total', count(*),
    ...
    'refunds', (select jsonb_build_object(
      'total', count(*),
      'pending', count(*) filter (where status in ('requested','under_review','approved','processing')),
      'completed', count(*) filter (where status = 'refunded'),
      'refunded_amount_cents', coalesce(sum(refund_amount_cents) filter (where status = 'refunded'), 0)
    ) from public.refund_requests),
    'notifications', (select jsonb_build_object(
      'pending', count(*) filter (where status in ('pending','processing','failed')),
      'sent', count(*) filter (where status = 'sent'),
      'dead_letter', count(*) filter (where dead_lettered_at is not null)
...
begin
  ...
  ) then raise exception 'superadmin_required'; end if;
  if p_limit < 1 or p_limit > 500 then raise exception 'invalid_audit_limit'; end if;

  return query
  select l.* from public.security_audit_log l
...
select cron.schedule(
  'hc20-security-cleanup',
  '17 3 * * *',
  $$select public.cleanup_security_operational_data(now());$$
);
