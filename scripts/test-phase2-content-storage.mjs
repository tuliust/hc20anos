...
const PASSWORD = "phase2-local-auth-password";
...
async function retryAuthOperation(operation, label, attempts = 10) {
  let lastError = null;
  for (let attempt = 1; attempt <= attempts; attempt += 1) {
    const result = await operation();
    if (!result.error) return result;
    lastError = result.error;
    const retryable = result.error.name === "AuthRetryableFetchError" || Number(result.error.status) >= 500;
    if (!retryable || attempt === attempts) break;
    await delay(attempt * 500);
  }
  assert.fail(`${label}: ${lastError?.message || JSON.stringify(lastError)}`);
}
...
async function createLocalUser(label, displayName, role = null) {
  const suffix = `${Date.now()}-${Math.random().toString(16).slice(2)}`;
  const email = `phase2-${label}-${suffix}@local.invalid`;
  const created = await retryAuthOperation(
    ...
    `Falha ao criar usuário ${label}`,
  );
  assert.ok(created.data.user?.id, `Auth local não retornou ID para ${label}`);
...
async function signedClient(user) {
  ...
  const signed = await retryAuthOperation(
    () => client.auth.signInWithPassword({ email: user.email, password: PASSWORD }),
    `Falha ao autenticar ${user.email}`,
  );
  assert.ok(signed.data.session?.access_token, `Sessão não criada para ${user.email}`);
  assert.equal(signed.data.user?.id, user.id);
  return { client, token: signed.data.session.access_token, id: user.id };
}
...
const createdUsers = [
  await createLocalUser("ordinary", "Usuário Fase 2"),
  await createLocalUser("moderator", "Moderador Fase 2", "moderator"),
  await createLocalUser("admin", "Administrador Fase 2", "admin"),
];
const ordinary = await signedClient(createdUsers[0]);
const moderator = await signedClient(createdUsers[1]);
...
try {
  ...
  assert.ok(photo.storage_path.startsWith(`${ordinaryId}/`));
  const listed = await service.storage.from("photos").list(ordinaryId);
  ...
  const downloaded = await service.storage.from("photos").download(photo.storage_path);
  assert.ifError(downloaded.error);
  assert.deepEqual(new Uint8Array(await downloaded.data.arrayBuffer()), PNG);
  ...
  const bucket = await service.from("rate_limit_buckets").select("request_count").eq("action", "photo_comment").eq("actor_user_id", ordinaryId).single();
  ...
  const removalAttempts = await Promise.all(Array.from({ length: 3 }, () => ordinary.client.rpc("submit_photo_removal_request", {
    ...
    p_requester_email: ordinary.email,
  ...
  const removalRows = await service.from("photo_removal_requests").select("id").eq("photo_id", photo.id).eq("requester_user_id", ordinaryId).in("status", ["pending", "hidden_preventively"]);
  ...
  const [photoAfter, requestAfter, commentsAfter, tagsAfter, eventsAfter, objectsAfter] = await Promise.all([
    ...
    service.storage.from("photos").list(ordinaryId),
  ]);
  ...
  await Promise.all(createdUsers.map(user => service.auth.admin.deleteUser(user.id).catch(() => null)));
}
