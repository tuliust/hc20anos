...
const OUTPUTS = {
  ...
  runtimeTypes: "src/lib/database.generated.ts",
};
const SOURCE_PATHS = [
...
function sourceMetadata() {
  ...
  const commit = gitValue(["log", "--no-merges", "-1", "--format=%H", ...pathArgs], "unknown");
  const date = gitValue(["log", "--no-merges", "-1", "--format=%cs", ...pathArgs], "1970-01-01");
  return { commit, date };
}
