...
function formatSubtitles(): void {
  ...
  document.querySelectorAll<HTMLElement>(SUBTITLE_SELECTOR).forEach(subtitle => {
    ...
    const [firstLine, secondLine] = splitSubtitle(text, card.getAttribute("data-ticket-product-code") ?? "");
...
function findSecurityPanel(): HTMLElement | null {
  ...
  const label = Array.from(main.querySelectorAll<HTMLElement>("h1,h2,h3,p,strong,span"))
    .find(element => normalize(element.textContent).toLocaleLowerCase("pt-BR").includes("compra segura via mercado pago"));
...
function injectStyles(): void {
  ...
  style.id = STYLE_ID;
  style.textContent = `
    [data-public-ticket-catalog="true"]:not([data-public-ticket-catalog-home="true"]) article[data-ticket-product-code] > div:first-child > div:first-child {
      display: flex; min-width: 0; flex: 1 1 auto; flex-direction: column;
    }
    [data-public-ticket-catalog="true"]:not([data-public-ticket-catalog-home="true"]) article[data-ticket-product-code] > div:first-child > div:first-child > p:first-child {
      order: 1; margin: 0 !important; line-height: 1.2;
    }
    [data-public-ticket-catalog="true"]:not([data-public-ticket-catalog-home="true"]) ${SUBTITLE_SELECTOR} {
      order: 2; display: block; min-height: 2.7em; margin: 1.25rem 0 0 !important; line-height: 1.35 !important;
    }
    [data-public-ticket-catalog="true"]:not([data-public-ticket-catalog-home="true"]) ${SUBTITLE_SELECTOR} > span[data-hc-subtitle-line] {
      display: block; min-height: 1.35em;
    }
    [data-public-ticket-catalog="true"]:not([data-public-ticket-catalog-home="true"]) article[data-ticket-product-code] h2 {
      order: 3; margin-top: .65rem !important; margin-bottom: 0 !important;
    }
    [data-public-ticket-catalog="true"]:not([data-public-ticket-catalog-home="true"]) article[data-ticket-product-code] > button {
