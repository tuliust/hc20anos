...
function updateHeaderExitButton() {
  ...
  const button = Array.from(
    document.querySelectorAll<HTMLButtonElement>(
      'main button[title="Voltar ao início"], button[title="Voltar ao início"]',
    ),
...
function updateGreetingClassLabel() {
  ...
  const classLabel = Array.from(greeting?.parentElement?.children ?? []).find(
...
function updateProfileName() {
  document
    .querySelectorAll<HTMLElement>(`[${PROFILE_NAME_ATTRIBUTE}]`)
    .forEach(element => {
      if (isAlumniAreaRoute()) return;
      ...
      if (element.dataset.alumniAreaWasTruncated === "true") {
...
>>>>>>> f7f58a6d80d88f0b6d9ef06d5fb5235875d15cab
  ...
  if (!name.hasAttribute(PROFILE_NAME_ATTRIBUTE)) {
    name.dataset.alumniAreaWasTruncated = String(
      name.classList.contains("truncate"),
    );
...
<<<<<<< HEAD
  ...
  document
    .querySelectorAll<HTMLElement>(`[${PROFILE_PHOTO_ATTRIBUTE}]`)
    .forEach(element => {
      [
        "width",
        "min-width",
  ...
  document
    .querySelectorAll<HTMLImageElement>(
      `img[${PROFILE_PHOTO_IMAGE_ATTRIBUTE}]`,
    )
    .forEach(image => {
      ["position", "inset", "width", "height", "object-fit"].forEach(property =>
...
<<<<<<< HEAD
  ...
  const row =
    profileLabel?.nextElementSibling instanceof HTMLElement
      ? profileLabel.nextElementSibling
      : null;
...
function updateTicketMessage() {
  ...
  const message = Array.from(
    ...
    element =>
      ...
      === "não foi possível conferir ingressos agora.",
  );
...
function updatePhotosHeaderLink() {
  ...
  const link = Array.from(
    photosCard?.querySelectorAll<HTMLButtonElement>("button") ?? [],
...
function updateEmptyStateActionBorders() {
  document
    .querySelectorAll<HTMLButtonElement>(
      `button[${ACTION_BORDER_ATTRIBUTE}]`,
    )
    .forEach(button => {
      if (isAlumniAreaRoute()) return;
