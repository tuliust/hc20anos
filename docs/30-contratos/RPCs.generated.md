...
| `public.get_contact_research_directory` | `—` | `TABLE(person_id uuid, full_name text, class_group text, phone text, instagram text, email text, notes text, research_status text, source text, updated_by uuid, updated_at timestamp with time zone)` | sim | `stable` | `postgres=X/postgres,authenticated=X/postgres,service_role=X/postgres,anon=X/postgres` |
...
| `public.save_contact_research` | `p_person_id uuid, p_phone text, p_instagram text, p_email text, p_notes text, p_source text, p_mark_no_contact boolean` | `alumni_contact_research` | sim | `volatile` | `postgres=X/postgres,authenticated=X/postgres,service_role=X/postgres,anon=X/postgres` |
